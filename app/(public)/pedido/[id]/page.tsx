...
const STEPS = [
  { label: "Pedido Liberado", description: "Pagamento confirmado e pedido liberado para preparo.", icon: "✓" },
  { label: "Pedido Montado", description: "Itens separados e conferidos pela farmácia.", icon: "📦" },
  { label: "Pedido Pronto para Coleta", description: "Aguardando retirada ou despacho.", icon: "🔴" },
];
...
export default async function PedidoPage({ params }: { params: Params }) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 animate-fade-in">
      ...
      <Card className="mt-6">
        <h2 className="mb-6 text-lg font-semibold">Status do pedido</h2>
      ...
      <Card className="mt-6">
          ...
          {items.map((it) => (
            <li key={it.id} className="flex items-center justify-between py-3">
              <div>
                ...
                <p className="text-sm text-muted-foreground">EAN {it.ean} · {it.quantidade} un.</p>
              </div>
              <span className="font-medium">{formatBRL(it.precoUnitCents * it.quantidade)}</span>
            </li>
          ))}
        </ul>
        <div className="mt-3 space-y-1 border-t border-border pt-3 text-sm">
          ...
          <div className="flex justify-between text-base font-bold"><span>Total</span><span>{formatBRL(order.precoTotalCents)}</span></div>
        </div>
      </Card>
